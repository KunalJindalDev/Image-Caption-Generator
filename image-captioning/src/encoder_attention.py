...
class EncoderAttentionCNN(nn.Module):
    ...
    def forward(self, images: Tensor) -> Tensor:
        ...
        if channels != 2048:
            raise ValueError(f"Expected 2048 channels from ResNet layer4, got {channels}.")

        # Rearrange spatial grid to sequence of locations: (B, H*W, C) = (B, 49, 2048).
        spatial_features = features.permute(0, 2, 3, 1).reshape(batch_size, height * width, channels)

        # Project each location embedding to encoder_dim.
        projected_features = self.projection(spatial_features)  # (B, 49, encoder_dim)
